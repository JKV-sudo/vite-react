...
function App() {
  ...
  useEffect(() => {
    ...
    const preload = async () => {
      ...
      // Track progress for each completed task
      ...
      // Monitor each task completion
      fontsReady.then(updateProgress);
      dynImports.then(updateProgress);
      images.then(updateProgress);
      dataPrefetch.then(updateProgress);
